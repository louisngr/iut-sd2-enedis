...
get_ademe_data <- function() {
  ...
  if (is.null(response) || response$status_code != 200) {
    warning("Impossible de récupérer les données ADEME (connexion, API ou timeout).")
    return(data.frame(
      emission_ges_chauffage = numeric(),
      nom_commune_brut = character(),
      code_departement_ban = character()
    ))
  }
