---
title: "Analyse comparative des consommations energetiques - DPE (Creuse, Correze, Haute-Vienne)"
author: "De Franceschi Lucas / Nugier Louis / Cerqueira Gregory - Equipe Greentech Solutions"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: false
...
Ce rapport, réalisé dans le cadre du projet Rshiny, vise à étudier l'association entre la **classe DPE** (Diagnostic de Performance Énergétique) et la **consommation d'énergie primaire** (kWh/m²/an) sur trois départements du massif central : **Creuse (23)**, **Corrèze (19)** et **Haute‑Vienne (87)**.  
...
**Source des données** : jeux DPE fournis par l'API ADEME (jeu `dpe03existant`) et pré‑traités via l'application Shiny de l'équipe. Pour exécuter ce rapport localement, on extrait directement les données via l'API ADEME.  
...
get_ademe_data_multi <- function(departements = c("23", "19", "87"),
  ...
  qs_filter <- paste0("code_departement_ban:(",
                      paste(quoted, collapse = " OR "), ")")

  params <- list(
    page = 1,
    size = size * length(departements),
    select = "etiquette_dpe,nom_commune_brut,code_departement_ban,
              surface_habitable_logement,conso_5_usages_par_m2_ep,
              type_batiment,annee_construction,etiquette_ges,_geopoint",
    qs = qs_filter
  )
  ...
  if (is.null(response) || response$status_code != 200) {
    message("Erreur API ADEME (statut: ", response$status_code, ").")
    return(NULL)
  }
...
departements <- unlist(strsplit(params$departements, ",")) |> trimws()

donnees <- get_ademe_data_multi(departements)

if (is.null(donnees) || nrow(donnees) == 0) {
stop("Aucune donnée n’a pu être récupérée via l’API ADEME.")
}
...
filter(!is.na(conso), !is.na(surface), conso > 0, surface > 0)
```
