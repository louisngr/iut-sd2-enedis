---
title: "Analyse comparative des consommations energetiques - DPE"
author: "De Franceschi Lucas / Nugier Louis / Cerqueira Gregory"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: false
...
get_ademe_data_multi <- function(departements = c("23", "19", "87"),
  ...
  results <- lapply(departements, function(dep) {
    query <- list(
      q = paste0("code_departement_ban:", dep),
      size = size
    )
    res <- httr::GET(full_url, query = query)
    httr::stop_for_status(res)
    jsonlite::fromJSON(httr::content(res, as = "text", encoding = "UTF-8"))$results
  })
...
Ce rapport, réalisé dans le cadre du projet *Enedis*, vise à étudier l'association entre la **classe DPE** (Diagnostic de Performance Énergétique) et la **consommation d'énergie primaire** (kWh/m²/an) sur trois départements du massif central : **Creuse (23)**, **Corrèze (19)** et **Haute‑Vienne (87)**.

L'approche adoptée est **exploratoire et comparative** : après présentation et préparation des données, nous produisons quelques indicateurs synthétiques (KPI) et trois visualisations ciblées permettant de répondre à la problématique principale 
**Dans quelle mesure la classe DPE et le type de bâtiment expliquent-ils la consommation ?** 

# Méthodologie et données

**Source des données** : jeux DPE fournis par l'API ADEME (jeu `dpe03existant`).
...
**Prétraitement** : typage numérique des variables pertinentes, retrait des valeurs absentes/non valides sur la consommation, création éventuelle d'agrégats par commune et découpage temporel par période de construction. Pour les nuages de points séléction sur 80% des données pour une meilleure lecture et outliers non présent sur les boxplots.

```{r data_prep}
# --- Paramètres : parsing des départements ---
deps <- strsplit(params$departements, ",")[[1]] %>% trimws()

# --- Chargement des données depuis l'API ADEME ---
donnees <- get_ademe_data_multi(
  departements = deps,
  size = 10000,
  dataset = "dpe03existant"
)
...
donnees <- donnees %>%
  ...
  filter(
    !is.na(conso),
    conso >= 0,
    code_departement_ban %in% deps
  )

# --- Filtre par type de bâtiment si demandé ---
if (tolower(params$type_batiment) != "tous") {
  donnees <- donnees %>%
    filter(type_batiment == tolower(params$type_batiment))
}

# --- Taille finale du jeu filtré ---
n_obs <- nrow(donnees)
...
kpi <- donnees %>%
  summarise(
    conso_moy_KWh_m2_an = round(mean(conso, na.rm = TRUE), 1),
    conso_med_KWh_m2_an = round(median(conso, na.rm = TRUE), 1),
    surface_moy_m2 = round(mean(surface, na.rm = TRUE), 1)
  )
...
**Interprétation** : la médiane et la dispersion par classe renseignent sur la capacité prédictive de la classe DPE ; une large dispersion au sein d'une même classe signale la présence d'autres facteurs explicatifs (surface, année de construction, chauffage...).

## 3. Corrélation surface <-> consommation (nuage de points + régression)

On examine la relation entre la surface habitable et la consommation. Un modèle de régression linéaire simple et ajusté pour quantifier l'association (R²).
